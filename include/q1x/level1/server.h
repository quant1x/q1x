...
namespace level1 {
    ...
    constexpr int _max_connections = 10;
    constexpr auto _max_elapsed_time = std::chrono::milliseconds(100).count();
    ...
    struct Server {
        ...
        constexpr auto serialize() {
            return std::tie(Source, Name, Host, Port, CrossTime);
        }
    };
    ...
    inline cista::offset::vector<Server> detect(i64 elapsed_time = _max_elapsed_time, int conn_limit = _max_connections) {
        runtime::global_init();
        // 创建 I/O 上下文
        asio::io_context io_context;
        asio::io_context::work work(io_context); // 防止 io_context.run() 提前退出
        std::vector<std::thread> io_threads_;
        // 创建线程池
        int num_threads = 5;
        for (int i = 0; i < num_threads; ++i) {
            io_threads_.emplace_back([&io_context]() {
                io_context.run();
            });
        }
        std::vector<Server> best_ips;
        for(auto const & v : StandardServerList) {
            ...
            try {
                ...
                try {
                    ...
                    if (connect_future.wait_for(std::chrono::seconds(1)) == std::future_status::timeout) {
                        socket.close();
        ...
        auto length = std::min(best_ips.size(), size_t(conn_limit));
