...
#define CXX_CHRONO_USE_DATE 1
#else
#define CXX_CHRONO_USE_DATE 0
static_assert(__cpp_lib_chrono >= 201907L, "C++20 <chrono> features not supported!");
#endif

#if CXX_CHRONO_USE_DATE
#include <date/date.h>
#include <date/tz.h>
...
namespace api {
    ...
    static std::string to_string(std::time_t tm, const char * const format = layout_only_date) {
        ...
        // 线程安全的方式获取本地时间
#ifdef _WIN32
        localtime_s(&local_time, &tm);
#else
        localtime_r(&tm, &local_time);
#endif
        char date_str[60] = {0};
        std::strftime(date_str, sizeof(date_str), format, &local_time);
