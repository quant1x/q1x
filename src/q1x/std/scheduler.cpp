...
runtime::task_id AsyncScheduler::schedule_cron(const std::string& name, const std::string& cron_expr, std::function<void()> task) {
    ...
    try {
        ...
        enqueue_task(ScheduledTask{
            first_run,
            [this, id, name] { execute_cron_task(id, name); },
            id,
            name
        });
...
void AsyncScheduler::scheduler_loop() {
    ...
    while (running_) {
        ...
        if (Clock::now() >= top_task.next_run) {
            ...
            //spdlog::debug("等待下一个任务执行时间: {}", std::chrono::duration_cast<std::chrono::milliseconds>(top_task.next_run - Clock::now()).count());
            // 带条件的超时等待
